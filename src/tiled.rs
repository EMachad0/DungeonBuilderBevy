...
use bevy::{
    ...
    log,
    prelude::{
        AddAsset, Added, AssetEvent, Assets, Bundle, Commands, Component, ComputedVisibility,
        DespawnRecursiveExt, Entity, EventReader, GlobalTransform, Handle, Image, Plugin, Query,
        Res, Transform, Vec3, Visibility,
    },
    reflect::TypeUuid,
    utils::HashMap,
};
            ...
            if let Some(tiled_map) = maps.get(map_handle) {
                ...
                for (tileset_index, tileset) in tiled_map.map.tilesets().iter().enumerate() {
                    ...
                    for (layer_index, layer) in tiled_map.map.layers().enumerate() {
                        ...
                        commands.entity(entity).add_child(layer_entity);
                        ...
                        let pivot =
                            Vec3::new(tile_size.x / 2., tile_size.y / 2., layer_index as f32);
